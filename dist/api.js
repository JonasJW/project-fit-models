...
export const fetchPresetsForRoutine = async (routine) => {
...
export const fetchRoutineStatus = async (uid, routineId) => {
...
export const fetchExerciseEntry = async (uid, id) => {
...
export const pushExerciseEntries = async (exercises, uid) => {
    const entryKeys = [];
...
export const pushFinishedWorkout = async (workout, uid) => {
    function replacer(key, value) {
...
export const pushRoutineStatus = async (status, workoutId, uid) => {
...
export const watchWorkoutHistory = async (uid, onChange) => {
...
export const watchRoutineStatuses = async (uid, onChange) => {
    firebase.firestore().collection('Users').doc(uid).collection('RoutineStatus').onSnapshot((query) => {
        const statuses = [];
        query.docs.forEach(async (doc) => {
            const status = doc.data();
            status.id = doc.id;
            const routine = await this.fetchRoutine(status.routineId);
...
// Put for Cms
export const setCategories = async (jsonString) => {
    try {
